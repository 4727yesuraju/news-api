...
export default function NewsPage({news}) {
  ...
  const dateTimeAgo = moment(new Date("2024-06-15T20:00:06Z")).fromNow();
        ...
        <>
            ...
            <div className="flex items-center text-lg">
              ...
              <div className="flex flex-col items-start">
                ...
                <span>by <span className="font-bold">{article.author ? article.author :  "john"}</span>
                </span>
              </div>
            </div>
            {<img
              src={article?.urlToImage ? article.urlToImage : '/newsError.jpg'}
              alt='article img'
              className='rounded-md w-full object-cover cursor-pointer -mt-1 mb-5'
            />}

            <pre className="text-wrap text-lg text-justify">
              {article.content.slice(0,article.content.indexOf('[')-2)}
